...
export default function ConnectWahooScreen() {
	...
	useEffect(() => {
		...
		async function handleInitialUrl() {
			try {
				...
				if (success === "wahoo_connected") {
					setMessage("Wahoo erfolgreich verbunden!");
					...
					setMessage(`Fehler bei der Verbindung zu Wahoo: ${error}`);
					setIsError(true);
				} else {
					...
					try {
						...
						if (status.success) {
							setMessage("Wahoo-Verbindung bestätigt");
				...
				<View className="flex-1 items-center justify-center px-6">
					{processing ? (
						<View className="items-center">
							...
							<Text className="text-base text-muted-foreground text-center">Verarbeite Wahoo-Verbindung...</Text>
						</View>
					) : (
