...
import {useHRCurve} from '@/hooks/useAnalytics';
import type {HRCurvePoint} from '@/services/analytics';
...
interface TouchInfo {
  x: number;
  duration: number;
  points: {key: string; color: string; bpm: number}[];
}

interface HRCurveChartProps {
  sport: 'cycling' | 'running';
}

export function HRCurveChart({sport}: HRCurveChartProps) {
  ...
  const allTime = useHRCurve(sport, 'all');
  const year = useHRCurve(sport, 'year');
  const days28 = useHRCurve(sport, '28d');
  ...
  const ranges = useMemo(() => [
    {key: 'all', labelKey: 'analytics.hrCurve.rangeAll', data: allTime.data?.hr_curve, color: colors.chart1, width: 2.5},
    {key: 'year', labelKey: 'analytics.hrCurve.rangeYear', data: year.data?.hr_curve, color: colors.chart2, width: 1.5},
    {key: '28d', labelKey: 'analytics.hrCurve.range28d', data: days28.data?.hr_curve, color: colors.chart3, width: 1.5},
  ...
  const {yMin, yMax, yTicks} = useMemo(() => {
    let max = 0;
    let min = Infinity;
    ranges.forEach((r) => r.data?.forEach((p) => {
      if (p.max_avg_bpm > max) max = p.max_avg_bpm;
      if (p.max_avg_bpm < min) min = p.max_avg_bpm;
    }));
    if (max === 0) return {yMin: 60, yMax: 200, yTicks: [60, 80, 100, 120, 140, 160, 180, 200]};
    const calcYMin = Math.max(40, Math.floor(min / 20) * 20 - 20);
    const calcYMax = Math.ceil(max / 20) * 20 + 10;
    const range = calcYMax - calcYMin;
    const step = Math.ceil(range / 6 / 10) * 10;
    const ticks = [];
  ...
  const yScale = useCallback((bpm: number): number => {
    const drawHeight = CHART_HEIGHT - PADDING_TOP - PADDING_BOTTOM;
    return PADDING_TOP + drawHeight * (1 - (bpm - yMin) / (yMax - yMin));
  }, [yMin, yMax]);

  const buildPath = useCallback((data: HRCurvePoint[]): string => {
    if (!data || data.length === 0 || chartWidth <= 0) return '';
    const sorted = [...data].sort((a, b) => a.duration_seconds - b.duration_seconds);
    return sorted.map((p, i) => `${i === 0 ? 'M' : 'L'}${logScale(p.duration_seconds).toFixed(1)},${yScale(p.max_avg_bpm).toFixed(1)}`).join(' ');
  }, [logScale, yScale, chartWidth]);

  const hasData = ranges.some((r) => r.data && r.data.length > 0);

  const findNearest = useCallback((data: HRCurvePoint[] | undefined, targetDuration: number): HRCurvePoint | null => {
    if (!data || data.length === 0) return null;
    let bestDist = Infinity;
    let best: HRCurvePoint | null = null;
    for (const p of data) {
  ...
  const processPosition = useCallback((x: number) => {
    ...
    for (const r of ranges) {
      ...
      if (nearest) points.push({key: r.key, color: r.color, bpm: nearest.max_avg_bpm});
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t('analytics.hrCurve.title')}</CardTitle>
        <Text className="text-sm text-muted-foreground">{t('analytics.hrCurve.subtitle')}</Text>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <View className="items-center justify-center py-12"><Spinner /></View>
        ) : isError ? (
          <Text className="text-muted-foreground text-center py-8">{t('analytics.hrCurve.error')}</Text>
        ) : !hasData ? (
          <Text className="text-muted-foreground text-center py-8">{t('analytics.hrCurve.noData')}</Text>
        ) : (
          <>
                  ...
                  {touchInfo && (
                    <>
                      ...
                      {touchInfo.points.map((p) => (
                        <Circle key={p.key} cx={touchInfo.x} cy={yScale(p.bpm)} r={4} fill={p.color} />
                      ))}
                      <Rect
                        x={tooltipX} y={0}
