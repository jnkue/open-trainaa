...
def main():
    ...
    profile.type_class_name_by_name = {
        k: convert_type_name(k) for k in profile.types_by_name.keys()
    }

    for message in profile.messages_by_name.values():
        message.field_class_name_by_name = {
            k: field_name_to_class_name(message, message.fields_by_name[k], k)
            for k in message.fields_by_name.keys()
        }
        message.field_property_name_by_name = {
            k: field_name_to_property_name(message, k)
            for k in message.fields_by_name.keys()
        }
        message.field_property_type_by_name = {
            k: get_field_property_type_name(profile, v)
        ...
        for field in message.fields_by_name.values():
            ...
            for sub_field in field.sub_fields:
                ...
                field.subfield_property_name_by_name[sub_field.name] = (
                    subfield_name_to_property_name(field, sub_field.name)
                )
                field.subfield_property_type_by_name[sub_field.name] = (
                    get_field_property_type_name(profile, sub_field)
                )

    for k, v in profile.types_by_name.items():
        profile_type = profile.types_by_name[k]
        profile_type.values_by_name = {
            convert_value_name(k): v for k, v in profile_type.values_by_name.items()
        }
    ...
    rendering = template.render(
        profile=profile, profile_types=profile_types, sdk_version=SDK_VERSION
    )
    ...
    for name, message in profile.messages_by_name.items():
        ...
        filename = os.path.join(
            messages_path, f"{inflection.underscore(message.name + '_message')}.py"
        )
        with open(filename, "w") as file_out:
            file_out.write(rendering)

    message_file_names = [
        inflection.underscore(message_name + "_message")
        for message_name in profile.messages_by_name.keys()
    ]
    message_class_names = [
        inflection.camelize(message_name + "_message")
        for message_name in profile.messages_by_name.keys()
    ]
